...
export default function linearGradientToBlack(svgGridItems, cb) {

	if (svgGridItems) {
		// set mapping for the grid items, 'r' stands for row
		let easing = Power1.easeInOut;
		let strokeColor = 'black';
		let fillColor = '#1c1c1c';

		// change the html bg color to black
		let htmlBody = document.querySelector('html');
		htmlBody.style.backgroundColor = 'black';


		let r1 = {
			styles: {fill: fillColor, ease: easing, stroke: strokeColor},
			items: [
		...
		let r10 = {
			styles: {
				...
				stroke: strokeColor,
				onComplete: () => {
					console.warn('black gradient onComplete Triggered')
					setTimeout(cb, 1000)
				}
			},
			items: [
